...
// 创建映射对象
const idToPthumbnailMap = {};

// 填充映射对象
data.forEach((item) => {
	idToPthumbnailMap[item.id] = item.pthumbnail;
});

// 映射函数
function getPthumbnailById(id) {
	return idToPthumbnailMap[id] || null; // 如果找不到对应的 id，则返回 null 或其他默认值
}

// 通过映射函数获取 pthumbnail
const exampleId = "034db61a861345f9bb681b7f283dd1e1";
const examplePthumbnail = getPthumbnailById(exampleId);

console.log(examplePthumbnail);
console.log(idToPthumbnailMap);
